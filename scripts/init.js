    ...
    actions: [
      ...
      // 初始化包信息
      {
        ...
        transform(data, input) {
          ...
          let { name, email, authorName, description, reopAuthorName } = input
      ...
      // 初始化 README 介绍
      {
        ...
        transform(data, { name, description, authorName }) {
          data = data.replace('vite-plugin-template', `vite-plugin-${name}`)
          data = data.replace('开箱即用的 vite 插件模板', description)
